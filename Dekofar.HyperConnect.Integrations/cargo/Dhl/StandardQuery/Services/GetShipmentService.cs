...
{
    public class GetShipmentService : IGetShipmentService
    {
        private readonly IConfiguration _config;
        private readonly IAuthService _authService;

        public GetShipmentService(IConfiguration config, IAuthService authService)
        {
            _config = config;
            _authService = authService;
        }

        public async Task<GetShipmentResponse> GetShipmentAsync(string referenceId)
        {
            // 🔑 Dinamik token al
            ...
            var client = new RestClient($"{baseUrl}/getshipment/{referenceId}");

            var request = BuildRequest(token);
            var response = await client.ExecuteAsync(request);

            if (!response.IsSuccessful)
                throw new Exception($"DHL GetShipment hatası: {response.StatusCode} - {response.Content}");

            return JsonSerializer.Deserialize<GetShipmentResponse>(
                response.Content!,
