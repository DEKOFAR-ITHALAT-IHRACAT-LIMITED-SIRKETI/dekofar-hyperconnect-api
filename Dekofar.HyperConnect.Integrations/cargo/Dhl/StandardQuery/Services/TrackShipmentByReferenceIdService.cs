...
{
    public class TrackShipmentByReferenceIdService : ITrackShipmentByReferenceIdService
    {
        private readonly IConfiguration _config;
        private readonly IAuthService _authService;

        public TrackShipmentByReferenceIdService(IConfiguration config, IAuthService authService)
        {
            _config = config;
            _authService = authService;
        }

        public async Task<List<TrackShipmentResponse>> TrackShipmentByReferenceIdAsync(string referenceId)
        {
            // 🔑 Dinamik token al
            ...
            var client = new RestClient($"{baseUrl}/trackshipment/{referenceId}");

            var request = BuildRequest(token);
            var response = await client.ExecuteAsync(request);

            if (!response.IsSuccessful)
                throw new Exception($"DHL TrackShipmentByReferenceId hatası: {response.StatusCode} - {response.Content}");
